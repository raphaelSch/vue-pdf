  ...
  switch (inputEl.type) {
    ...
    case "checkbox":
      return buildAnnotationData(FORM_CHECKBOX, {
        ...
        exportValue: inputEl.exportvalue,
      });
    case "radio":
    ...
    if (typeof annotation.dest === "string") {
      return buildAnnotationData(INTERNAL_LINK, {
        referencedPage: Number(
          annotation.dest.substring(1, annotation.dest.length)
        ),
        offset: null,
      });
    } else {
      const pageIndex = await PDFDoc.getPageIndex(
        annotation.dest[0] as RefProxy
      );
      return buildAnnotationData(INTERNAL_LINK, {
  ...
  if (annotation.className === "linkAnnotation" && evt.type === "click") {
    ...
    if (id)
      return linkAnnotation(
        getAnnotationsByKey("id", id, Annotations)[0] as LinkAnnotation,
        PDFDoc
      );
  } else if (
    annotation.className.includes("popupAnnotation") ||
    annotation.className.includes("textAnnotation")
  ) {
    ...
    annotation.className.includes("textWidgetAnnotation") &&
    (evt.type === "input" || evt.type === "click")
  ) {
    let inputElement: HTMLInputElement | HTMLTextAreaElement =
      annotation.getElementsByTagName("input")[0];
    if (!inputElement)
      inputElement = annotation.getElementsByTagName("textarea")[0];
    return inputAnnotation(inputElement);
  } else if (
    annotation.className.includes("choiceWidgetAnnotation") &&
    evt.type === "click"
  ) {
    return inputAnnotation(annotation.getElementsByTagName("select")[0]);
  } else if (
    annotation.className.includes("buttonWidgetAnnotation checkBox") &&
    evt.type === "change"
  ) {
    return inputAnnotation(annotation.getElementsByTagName("input")[0]);
  } else if (
    annotation.className.includes("buttonWidgetAnnotation radioButton") &&
    evt.type === "change"
  ) {
    const id = annotation.dataset.annotationId;
    if (id) {
    ...
    annotation.className.includes("buttonWidgetAnnotation pushButton") &&
    evt.type === "click"
  ) {
    const id = annotation.dataset.annotationId;
    if (id) {
